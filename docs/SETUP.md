...
5. First install only: the app won't launch until you trust your developer certificate on the
   phone. Go to **Settings → General → VPN & Device Management** → your Apple ID → **Trust**.
...
## Optional: adding the Screen Time hard-block targets

The Screen Time hard-block needs two new app extension targets before any code can be
written. Xcode has to generate them (new build phases, provisioning, an embed-extensions step on the host app) so
...
edge: unlike the web client, a lapsed signature can leave native Instagram shielded with no
